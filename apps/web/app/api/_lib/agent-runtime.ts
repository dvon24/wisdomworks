...
async function callAnthropicForTick(
  ...
  // Force DIRECT tool calling for ticks. This loop sets the PTC beta header
  // but never adds the code_execution tool or threads the container_id across
  // iterations, so a tool that opts into PTC via allowed_callers would 400 the
  // 2nd iteration with "container_id is required when there are pending tool
  // uses generated by code execution with tools." Same fix as the
  // delegate_to_agent worker loop — ticks are bounded reasoning tasks where
  // PTC's token savings don't justify replicating container-threading here.
  const directTools = tools.map((t) => {
    if (Array.isArray(t?.allowed_callers)) { const { allowed_callers: _drop, ...rest } = t; return rest; }
    return t;
  });
  ...
  const buildBody = (): any => {
    ...
    if (hasTools) {
      ...
      body.tools = directTools.map((t, i) =>
        i === directTools.length - 1 ? { ...t, cache_control: { type: 'ephemeral' } } : t,
      );
    }
    return body;
  };

  // Always false now — directTools strips allowed_callers above. Kept as a
  // guard so if that strip is ever removed, the beta header tracks correctly.
  const usePtc = hasTools && directTools.some((t) => Array.isArray(t.allowed_callers) && t.allowed_callers.includes('code_execution_20250825'));
