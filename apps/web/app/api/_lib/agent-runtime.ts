...
async function callAnthropicForTick(
  ...
  // Resolve the user context executeTool needs for this tenant's tools (only
  // when tools are present — most no-tool ticks don't need it).
  ...
  // Shared sub-agent loop — PTC handled in ONE place. enablePtc:false → direct
  // tool calling, so a tick tool that opts into PTC via allowed_callers can't
  // 400 the loop ("container_id is required when there are pending tool uses
  // generated by code execution with tools"). This consolidation stops every
  // sub-agent call site from reintroducing that bug as more agents are added.
  const { runAgentToolLoop } = await import('./anthropic-agent-loop');
  const run = await runAgentToolLoop({
    apiKey: ANTHROPIC_API_KEY!,
    model,
    system: systemPrompt,
    initialMessage: 'Run your scheduled tick.',
    tools,
    maxIterations: MAX_TICK_ITERATIONS,
    maxTokens: hasTools ? 4000 : 400,
    connections,
    userContext: userCtxForTools,
    enablePtc: false,
  });
  if (run.apiError) {
    throw new Error(`Anthropic ${run.apiError.status}: ${run.apiError.rawBody}`);
  }
  const finalText = run.finalText;
  const totalTokensIn = run.tokensIn;
  const totalTokensOut = run.tokensOut;
